...
def main():
    ...
    if args.do_test:
        ...
        for criteria in ['best-acc']:
            ...
            # Check if the state_dict is from a DataParallel model or not
            from_data_parallel = list(state_dict.keys())[0].startswith('module.')

            # Adjust the state_dict keys accordingly
            if from_data_parallel and not isinstance(model, torch.nn.DataParallel):
                state_dict = {k[len("module."):]: v for k, v in state_dict.items()}
            elif not from_data_parallel and isinstance(model, torch.nn.DataParallel):
