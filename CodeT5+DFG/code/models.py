...
import time
from Layers import GraphEmbedding, PredictionClassification
...
class DefectModel(nn.Module):
    def __init__(self, encoder, config, tokenizer, args):
        ...
        self.config = config
        self.tokenizer = tokenizer
        self.graphEmb = GraphEmbedding(feature_dim_size=config.hidden_size, hidden_size=256, dropout=0.2)
        # self.classifier = nn.Linear(config.hidden_size + self.graphEmb.out_dim, 2)
        self.classifier = PredictionClassification(config, args, input_size=self.graphEmb.out_dim + config.hidden_size)
        self.args = args
        self.w_embeddings = self.encoder.shared.weight.data.cpu().detach().clone().numpy()
    ...
    def forward(self, source_ids=None, labels=None):
        adj, x_feature = build_dfg(source_ids.cpu().detach().numpy(), self.w_embeddings, self.tokenizer)
        ...
        outputs = self.classifier(torch.cat((vec,g_emb.float()), dim=1))
        logits = outputs
        prob = nn.functional.softmax(logits)
